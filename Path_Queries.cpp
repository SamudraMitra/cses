...
ll dfs(ll node, ll par, vector<vector<ll>> &adj, ll n, ll sumtillnow, vector<ll> &v, vector<pair<ll, ll>> &dp, vector<ll> &sot, vector<ll> &dfsorder)
{
    dp[node - 1].second = sumtillnow + v[node - 1];
    ll res = 0;
    ...
    for (auto &ch : adj[node])
    {
        ...
        res += dfs(ch, node, adj, n, sumtillnow + v[node - 1], v, dp, sot, dfsorder);
    }
    sot[node] = res + 1;
    return sot[node];
}
int main()
{
    ...
    vector<ll> sot(n + 1, 0);
    vector<ll> dfsorder;
    dfs(1, -1, adj, n, 0ll, v, dp, sot, dfsorder);
    vector<ll> idx(n + 1, 0);
    for (ll i = 0; i < n; i++)
    {
        idx[dfsorder[i]] = i;
    ...
    for (ll i = 0; i < n; i++)
    {
        segvec[i] = dp[dfsorder[i] - 1].second;
    }

    for (ll i = n - 1; i >= 1; i--)
    {
        segvec[i] = segvec[i] - segvec[i - 1];
    }
    // for(auto &i: segvec)
    // cout<<i<<" ";
    // cout<<"\n-----\n";
    // dp.push_back(0);
    SegmentTree<ll> seg(segvec, 0, combine);
    while (q--)
    {
        ll ty;
        cin >> ty;
        if (ty == 1)
        {
            ...
            ll li = idx[s];
            ll hi = idx[s] + sot[s];
            seg.update(li, diff + seg.query(li, li));
            seg.update(hi, seg.query(hi, hi) - diff);
        }
        else
        {
            ll x;
            cin >> x;
            ll res = seg.query(0, idx[x]);
            cout << res << "\n";
        }
    }
}
