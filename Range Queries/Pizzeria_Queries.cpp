...
int main()
{
    ...
    SegmentTree<pair<ll, ll>> seg(v, {LLONG_MAX, LLONG_MAX}, combine);
    while (q--)
    {
        ll type;
        cin >> type;
        if (type == 1)
        {
            ll a, b;
            cin >> a >> b;
            seg.update(a - 1, {b + (a - 1), b - (a - 1)});
        }
        else
        {
            ll x;
            cin >> x;
            ll z1 = seg.query(x - 1, n - 1).first;
            ll z2 = seg.query(0, x - 1).second;
            ll res = min(z1 - (x - 1), z2 + (x - 1));
            // cout << z1 << " " << z2 << "\n";
            cout << res << "\n";
        }
    }
