...
pair<pair<ll, ll>, pair<ll, ll>> combine(pair<pair<ll, ll>, pair<ll, ll>> a, pair<pair<ll, ll>, pair<ll, ll>> b)
{
    pair<pair<ll, ll>, pair<ll, ll>> res = {{max3(a.first.first, b.first.first, a.first.second + b.second.first),
                                             max(b.second.second + a.first.second, b.first.second)},
                                            {max(a.second.second + b.second.first, a.second.first),
...
int main()
{
    ...
    for (auto &i : v)
    {
        ll x;
        cin >> x;
        i = {{max(x, 0ll), max(x, 0ll)}, {max(x, 0ll), x}};
        // i[0] = max(i[0], x);
        // i[1] = max(i[1], x);
        // i[2] = max(i[2], x);
        // i[3] = x;
    }
    SegmentTree<pair<pair<ll, ll>, pair<ll, ll>>> seg(v, {{0ll, 0ll}, {0ll, 0ll}}, combine);
    while (q--)
    {
        ll x, y;
        cin >> x >> y;
        x--;
        pair<pair<ll, ll>, pair<ll, ll>> rep = {{max(0ll, y), max(0ll, y)}, {max(0ll, y), y}};
        seg.update(x, rep);
        cout << seg.query(0, n - 1).first.first << "\n";
    }
}
