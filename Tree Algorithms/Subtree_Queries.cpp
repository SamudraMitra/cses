...
ll f(ll node, ll par, vector<vector<ll>> &adj, ll n, vector<ll> &sos, vector<ll> &dfs)
{
    ll res = 1;
    if (sos[node] != -1)
        return sos[node];
...
int main()
{
    ...
    while (q--)
    {
        ...
        else
        {
            ll x;
            cin >> x;
            ll res = seg.query(mp[x], mp[x] + sos[x] - 1);
            cout << res << "\n";
        }
    }
}