...
ll dfs(ll node, ll par, vector<vector<ll>> &adj, vector<ll> &dfsorder, vector<ll> &sost)
{
    ...
    for (auto &ch : adj[node])
    {
        if (par == ch)
            continue;
        res += dfs(ch, node, adj, dfsorder, sost);
    }
    return sost[node] = res;
}
int main()
{
    fastio;
    ...
    vector<ll> DFSORDER;
    vector<ll> sost(N + 1, 0);
    dfs(1, -1, ADJ, DFSORDER, sost);
    vector<ll> IDX(N + 1, 0);
    for (ll i = 0; i < N; i++)
    {
        IDX[DFSORDER[i]] = i;
